...
extension IBaseHandler {
    func apiHandler <T: KotlinBase>(apiCall : () async throws -> Result ) async -> BaseResult<T?>{
       ...
       if response is ResultSuccess<T> {
           ...
           baseResult =  BaseResult<T?>(isSuccessful: false , data: res?.data , error: nil)
           } else {
               let err = response as? ResultError<NetworkError>
               baseResult = BaseResult<T?>(isSuccessful: false , data: nil, error: err?.error)
           }
        return baseResult
    }
}
